...
public sealed class JyroString : JyroValue
{
    ...
    public override JyroValue EvaluateBinary(int @operator, JyroValue right)
    {
        ...
        if (right is JyroString rightString)
        {
            ...
            {
                JyroParser.LT => JyroBoolean.FromBoolean(string.Compare(Value, rightString.Value, StringComparison.Ordinal) < 0),
                JyroParser.LE => JyroBoolean.FromBoolean(string.Compare(Value, rightString.Value, StringComparison.Ordinal) <= 0),
                JyroParser.GT => JyroBoolean.FromBoolean(string.Compare(Value, rightString.Value, StringComparison.Ordinal) > 0),
                JyroParser.GE => JyroBoolean.FromBoolean(string.Compare(Value, rightString.Value, StringComparison.Ordinal) >= 0),
