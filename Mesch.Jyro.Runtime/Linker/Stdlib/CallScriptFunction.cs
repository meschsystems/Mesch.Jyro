...
public sealed class CallScriptFunction : JyroFunctionBase
{
    ...
    public override JyroValue Execute(IReadOnlyList<JyroValue> arguments, JyroExecutionContext executionContext)
    {
        ...
        try
        {
            ...
            var lexer = new Lexer(Microsoft.Extensions.Logging.Abstractions.NullLogger<Lexer>.Instance);
            var parser = new Parser(Microsoft.Extensions.Logging.Abstractions.NullLogger<Parser>.Instance);
            var validator = new Validator(Microsoft.Extensions.Logging.Abstractions.NullLogger<Validator>.Instance, executionContext.Functions.Keys);
            var linker = new Linker(Microsoft.Extensions.Logging.Abstractions.NullLogger<Linker>.Instance);
            var executor = new Executor(Microsoft.Extensions.Logging.Abstractions.NullLogger<Executor>.Instance);

            var lexingResult = lexer.Tokenize(scriptSource.Value);
            ...
            var validationResult = validator.Validate(parsingResult.Statements);
            if (!validationResult.IsSuccessful)
            {
                return dataArgument;
            }

            var linkingResult = linker.Link(parsingResult.Statements, executionContext.Functions.Values);
            ...
            try
            {
                var executionResult = executor.Execute(linkingResult.Program, executionContext);
