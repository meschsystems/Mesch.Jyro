...
public abstract class JyroValue : IEquatable<JyroValue>
{
    ...
    /// <summary>
    /// Evaluates a binary operation with this value as the left operand.
    /// Supports arithmetic, comparison, logical, and equality operations based on the value types involved.
    /// </summary>
    ...
    /// <exception cref="InvalidOperationException">Thrown when the operation is not supported for the given types.</exception>
    public virtual JyroValue EvaluateBinary(JyroTokenType @operator, JyroValue right)
    {
        return @operator switch
        {
            JyroTokenType.EqualEqual => JyroBoolean.FromBoolean(this.Equals(right)),
            JyroTokenType.BangEqual => JyroBoolean.FromBoolean(!this.Equals(right)),
            JyroTokenType.And => JyroBoolean.FromBoolean(this.ToBooleanTruthiness() && right.ToBooleanTruthiness()),
            JyroTokenType.Or => JyroBoolean.FromBoolean(this.ToBooleanTruthiness() || right.ToBooleanTruthiness()),
            _ => throw new InvalidOperationException($"Unsupported binary operation {@operator} for {Type}")
        };
    }

    /// <summary>
    /// Evaluates a unary operation with this value as the operand.
    /// Supports logical negation and arithmetic negation based on the value type.
    /// </summary>
    /// <param name="operator">The unary operator to apply.</param>
    /// <returns>The result of the unary operation.</returns>
    /// <exception cref="InvalidOperationException">Thrown when the operation is not supported for this value type.</exception>
    public virtual JyroValue EvaluateUnary(JyroTokenType @operator)
    {
        return @operator switch
        {
            JyroTokenType.Not => JyroBoolean.FromBoolean(!this.ToBooleanTruthiness()),
